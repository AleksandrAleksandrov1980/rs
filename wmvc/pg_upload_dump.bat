...
SET server="msk-N9E-PSU5.ntc.ntcees.ru"
SET server_port=5432
SET new_db="rastrwin"
SET db_dump_file="C:\projects\git_main\rs\wmvc\pg_dump.bin"
"C:\Program Files\PostgreSQL\16\bin\dropdb"     -h %server% -p %server_port% -U postgres --if-exists  %new_db%
"C:\Program Files\PostgreSQL\16\bin\createdb"   -h %server% -p %server_port% -U postgres -T template0 %new_db%
"C:\Program Files\PostgreSQL\16\bin\pg_restore" -h %server% -p %server_port% -U postgres -d %new_db% %db_dump_file%
ECHO "That's all!"